...
bool JackAudioDriver::open(const Spec& spec, Spec* activeSpec)
{
    ...
    jack_port_t* output_port_left = jack_port_register(handle, "audio_out_left",
                                                       JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput,
                                                       0);
    s_jackData->outputPorts.push_back(output_port_left);
    jack_port_t* output_port_right = jack_port_register(handle, "audio_out_right",
                                                        JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput,
                                                        0);
    ...
    if (spec.output.sampleRate != jackSamplerate) {
        LOGW() << "Musescores samplerate: " << spec.output.sampleRate <<
        ", is NOT the same as jack's: " << jackSamplerate;
