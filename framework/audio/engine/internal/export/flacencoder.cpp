...
protected:
    FLAC__StreamEncoderWriteStatus write_callback(const FLAC__byte buffer[], const size_t bytes,
...
size_t FlacEncoder::encode(const samples_t samplesPerChannel, const float* input)
{
    ...
    for (samples_t frameStart = 0; frameStart < samplesPerChannel;) {
        const uint32_t framesCount
            = static_cast<uint32_t>(std::min<samples_t>(BLOCK_FRAMES,
                                                        samplesPerChannel - frameStart));
