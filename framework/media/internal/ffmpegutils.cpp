...
static FFmpegLibPaths libraryPathsForVersion(int ffmpegVer, const io::path_t& searchDir)
{
...
#if defined(Q_OS_MAC)
    avutilName = io::path_t("libavutil." + std::to_string(
                                ffmpegVersionInfo.avUtilVersion) + ".dylib");
    avcodecName = io::path_t("libavcodec." + std::to_string(
                                 ffmpegVersionInfo.avCodecVersion) + ".dylib");
    avformatName = io::path_t("libavformat." + std::to_string(
                                  ffmpegVersionInfo.avFormatVersion) + ".dylib");
    swscaleName = io::path_t("libswscale." + std::to_string(
                                 ffmpegVersionInfo.swScaleVersion) + ".dylib");
    swresampleName
        = io::path_t("libswresample." + std::to_string(
                         ffmpegVersionInfo.swResampleVersion) + ".dylib");
#elif defined(Q_OS_LINUX) || defined(Q_OS_FREEBSD)
    ...
    avformatName
        = io::path_t("libavformat.so." + std::to_string(ffmpegVersionInfo.avFormatVersion));
    swscaleName = io::path_t("libswscale.so." + std::to_string(ffmpegVersionInfo.swScaleVersion));
    swresampleName
        = io::path_t("libswresample.so." + std::to_string(ffmpegVersionInfo.swResampleVersion));
#elif defined(Q_OS_WIN)
    avutilName = io::path_t("avutil-" + std::to_string(ffmpegVersionInfo.avUtilVersion) + ".dll");
    avcodecName
        = io::path_t("avcodec-" + std::to_string(ffmpegVersionInfo.avCodecVersion) + ".dll");
    avformatName = io::path_t("avformat-" + std::to_string(
                                  ffmpegVersionInfo.avFormatVersion) + ".dll");
    swscaleName
        = io::path_t("swscale-" + std::to_string(ffmpegVersionInfo.swScaleVersion) + ".dll");
    swresampleName = io::path_t("swresample-" + std::to_string(
                                    ffmpegVersionInfo.swResampleVersion) + ".dll");
#endif
