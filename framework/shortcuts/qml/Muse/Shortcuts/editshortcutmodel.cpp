...
void EditShortcutModel::checkNewSequenceForConflicts()
{
    ...
    for (const QVariant& shortcut : m_potentialConflictShortcuts) {
        ...
        std::vector<std::string> toCheckSequences
            = Shortcut::sequencesFromString(map.value("sequence").toString().toStdString());
...
{
    std::vector<std::string> sequences = Shortcut::sequencesFromString(
        m_originSequence.toStdString());
