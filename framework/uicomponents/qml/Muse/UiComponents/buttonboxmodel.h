...
class ButtonBoxModel : public QObject
{
    ...
    Q_PROPERTY(
        ButtonLayout buttonLayout READ buttonLayout WRITE setButtonLayout NOTIFY buttonLayoutChanged)

    Q_PROPERTY(
        QQmlListProperty<QQuickItem> buttonsItems READ buttonsItems NOTIFY buttonsItemsChanged)
...
signals:
    ...
    void addButton(QString text, int buttonType, ButtonRole buttonRole, bool isAccent,
                   bool isLeftSide);
...
private:
    ...
    struct LayoutButton {
        ...
        LayoutButton(QString _text, int _buttonType,  ButtonRole _buttonRole, bool _isAccent,
                     bool _isLeftSide = false)
            : text(_text), buttonType(_buttonType),  buttonRole(_buttonRole), isAccent(_isAccent),
    ...
    QHash<ButtonType, LayoutButton*> m_layoutButtons {
        ...
        { SaveAll,         new LayoutButton(muse::qtrc("uicomponents",
                                                       "Save all"),         SaveAll,         ApplyRole,
                                            false) },
        { DontSave,        new LayoutButton(muse::qtrc("uicomponents",
                                                       "Don’t save"),       DontSave,        DestructiveRole,
                                            false) },
        ...
        { RestoreDefaults, new LayoutButton(muse::qtrc("uicomponents",
                                                       "Restore defaults"), RestoreDefaults, ResetRole,
                                            false) },
        { Continue,        new LayoutButton(muse::qtrc("uicomponents",
                                                       "Continue"),         Continue,        ContinueRole,
                                            true) },
        { Next,            new LayoutButton(muse::qtrc("uicomponents",
                                                       "Next"),             Next,            ContinueRole,
                                            false) },
        { Back,            new LayoutButton(muse::qtrc("uicomponents",
                                                       "Back"),             Back,            BackRole,
                                            false) },
        { Select,          new LayoutButton(muse::qtrc("uicomponents",
                                                       "Select"),           Select,          AcceptRole,
                                            true) },
        { Clear,           new LayoutButton(muse::qtrc("uicomponents",
                                                       "Clear"),            Clear,           DestructiveRole,
                                            false) },
        { Done,            new LayoutButton(muse::qtrc("uicomponents",
                                                       "Done"),             Done,            AcceptRole,
                                            true) }
    };

    std::vector<std::vector<ButtonRole> > buttonRoleLayouts = {
        // WinLayout
        std::vector <ButtonRole> { CustomRole, ResetRole, RetryRole, BackRole, AcceptRole,
                                   ApplyRole, ContinueRole, DestructiveRole,
                                   RejectRole, HelpRole },

        // MacLayout
        std::vector <ButtonRole> { CustomRole, HelpRole, RetryRole, DestructiveRole, RejectRole,
                                   ResetRole, BackRole, AcceptRole,
